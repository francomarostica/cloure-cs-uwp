...
namespace Cloure.Modules.users
{
    ...
    public sealed partial class UsersPage : Page
    {
        ...
        {
            UsersResponse response = await Users.getList(Filter, OrderBy, OrderType, Page);
            ...
            txtRegister.Text = response.PageString;
        ...
        private void lstItems_RightTapped(object sender, RightTappedRoutedEventArgs e)
        {
            ...
            User item = (User)((FrameworkElement)e.OriginalSource).DataContext;
        ...
        private void MenuFlyoutItem_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (menuFlyoutItem.Name == "add_payment") CloureManager.Navigate(typeof(UserAddPaymentPage), item.id);
            if (menuFlyoutItem.Name == "account_summary") CloureManager.Navigate(typeof(UserCCPage), item.id);
            if (menuFlyoutItem.Name == "generate_random_pass") DisplayRegenatePass(item);
        }
        ...
        private async void DisplayDeleteDialog(User item)
        {
            ...
            {
                bool api_result = await Users.Delete(item.id);
                if (api_result) LoadData();
            }

        }

        private async void DisplayRegenatePass(User item)
        {
            ContentDialog deleteFileDialog = new ContentDialog
            {
                Title = "¿Está seguro que desea reasignar la clave del usuario?",
                Content = "La clave se generará de forma aleatoria",
            ...
            if (result == ContentDialogResult.Primary)
            {
                bool api_result = await Users.GeneratePass(item.id);
            }
